...
@test Overt.add_var(OverApproximation()) == :v_1

@test Overt.apply_fx(:(x + y), :a) == :(a + y)

@test Overt.is_number(:(log(2)/3))

@test Overt.is_unary(:(sin(x)))
@test ! Overt.is_unary(:(x + y))

@test Overt.is_binary(:(x + y))

@test Overt.is_affine(:(x+1))
@test Overt.is_affine(:(-x+(2y-z)))
@test ! Overt.is_affine(:(log(x)))
@test ! Overt.is_affine(:(x + x*z))
@test Overt.is_affine(:(x/6))
@test Overt.is_affine(:(5*x))
@test Overt.is_affine(:((1 / 6) * x))
@test Overt.is_affine(:(log(2)*x))
@test Overt.is_affine(:(-x))
...
b1.nvars = 2
Overt.expand_multiplication(:A, :B, b1; ξ=0.1)
...
Overt.bound_binary_functions(:*, :A, :B, b1)

xxx = 0; yyy = 1; zzz = 2; kkk = -10;
e1 = :(xxx + yyy + zzz + kkk - xxx - yyy - zzz - kkk)
e2 = Overt.reduce_args_to_2(e1)
